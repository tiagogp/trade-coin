...
const CoinItem: React.FC<CoinRowProps> = ({ coin, index, currency }) => {
  if (!coin) return null
    ...
    >
      <div className='flex items-center flex-1'>
        ...
        <div className='flex flex-1 justify-end items-center gap-2'>
          {formatNumber(Number(coin.current_price), currency)}
          <Percentage value={Number(coin.price_change_percentage_24h)} />
        </div>
        <div className='hidden md:block  flex-1 text-right'>
          {formatNumber(Number(coin.market_cap), currency)}
        </div>
        <div className='hidden md:block  flex-1 text-right'>
          {formatNumber(Number(coin.total_volume), currency)}
        </div>
